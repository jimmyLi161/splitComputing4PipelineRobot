...
def evaluate(dataloader, model, device, iljx):
    model.eval()
    ...
    with torch.no_grad():
        for i, (images, imgPaths) in enumerate(dataloader):
            ...
            a = output.cpu()
            np.save("D:\\TJU\\test\\Nt\\npy\\{}_opm.npy".format(eljx),a)
            eljx = eljx + 1
            print("3")            
...
def run_inference(args):
    ...
    print("VALIDATION")
    iljx = 0
    sigmoid_predictions, val_imgPaths = evaluate(dataloader, model, device, iljx)
    print("A")
    sigmoid_dict = {}
    print("B")    
